...
GLOBAL void calc_fully_connected(
        Connection conn,
        float* inputs, float* weights, OUT* outputs,
        float(*extractor)(OUT, ARGS...), ARGS... args) {
    int col = blockIdx.x * blockDim.x + threadIdx.x;

    if (col < conn.to_size) {
        float sum = 0;
        for (int row = 0 ; row < conn.from_size ; ++row) {
            sum += extractor(outputs[row], args...)
                * weights[row * conn.to_size + col];
        }
...
GLOBAL void calc_one_to_one(
    ...
    if (index < conn.from_size) {
        inputs[index] = calc(conn.opcode, inputs[index],
            extractor(outputs[index], args...) * weights[index]);
    }
}
...
GLOBAL void calc_divergent(
    ...
    if (d_row < conn.to_rows and d_col < conn.to_columns) {
        ...
        int kernel_row_size = (conn.convolutional)
                              ? 1 : conn.from_rows * conn.from_columns;
        ...
        for (int s_row = start_s_row ; s_row <= end_s_row ; ++s_row) {
            for (int s_col = start_s_col ; s_col <= end_s_col ; ++s_col) {
                ...
                int k_row =
                    (d_row + ((conn.overlap - conn.stride) * s_row)
                    % conn.overlap);
                int k_col =
                    (d_col + ((conn.overlap - conn.stride) * s_col)
                    % conn.overlap);

                // Row of matrix is the kernel index * row size (see above)
                int weight_offset =
                    ((k_row * conn.overlap) + k_col)
                    * kernel_row_size;
                // Column of matrix is either the first column (convolutional)
                //   or the index of the source neuron otherwise
                int weight_col = (conn.convolutional)
                                 ? 0 : s_index;

                sum += extractor(outputs[s_index], args...) *
...
GLOBAL void calc_convergent(
    ...
    if (d_row < conn.to_rows and d_col < conn.to_columns) {
        ...
        int kernel_size = conn.overlap * conn.overlap;
        int kernel_row_size = (conn.convolutional)
                              ? 1 : conn.to_rows * conn.to_columns;
...
GLOBAL void calc_fully_connected(
    ...
    for (int row = 0 ; row < conn.to_size ; ++row) {
        ...
        for (int col = 0 ; col < conn.from_size ; ++col) {
            sum += extractor(outputs[col], args...) *
